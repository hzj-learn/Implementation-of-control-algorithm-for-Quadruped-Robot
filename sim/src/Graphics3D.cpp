...
//注意：我们需要创建一个旋转，使z轴指向“方向”的方向论证。
//因此，我们可以实现纯x-y轴的旋转旋转，旋转旋转帧“r”到当前帧“c”的旋转矩阵（c\u r\u r）具有特殊形式。
//它的第三列的形式是：[wy*sin（theta）-wx*sin（theta）（1-cos（theta））]，其中θ，[wx wy0]是旋转轴的角度

//通过比较当前帧和旋转帧的z轴来找到旋转角度
  const double cosTheta = dzn;
  const double theta = acos(cosTheta);
  const double sinTheta = sin(theta);
//利用旋转矩阵的特殊形式（如上所述）来寻找旋转轴
  float rX, rY, rZ;
  if (theta > 0) {
    rX = -dyn / sinTheta;
    rY = dxn / sinTheta;
