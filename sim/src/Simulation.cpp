...
void Simulation::sendControlParameter(const std::string& name,
  ...
  if (_sharedMemory().waitForRobotWithTimeout()) {
    ...
    handleControlError();
    request.requestNumber = response.requestNumber;//所以如果我们回来就不会在1点前关闭
    _robotMutex.unlock();
    return;
  }
...
void Simulation::buildLcmMessage() {
  ...
  for (size_t i = 0; i < 3; i++) {
    _simLCM.vb[i] = state.bodyVelocity[i + 3];  // 骨架线速度
    _simLCM.rpy[i] = rpy[i];
 ...
 * @param resti       : 恢复系数
 * @param height      : 平面高度
 * @param addToWindow : 如果为真，还将为平面添加图形
 */
...
/*!
 *在当前线程中运行模拟器，直到设置了 running变量
 *错了。如果需要，以每秒60帧的速度更新图形。在运行模拟
 *期望速度
 * @param dt
 */
void Simulation::runAtSpeed(std::function<void(std::string)> errorCallback, bool graphics) 
{
  ...
  //（5）若运行机器人标志位开启，死循环运行开始【重要】
  //1）提示仿真的低级运行时间、高级运行时间和图形更新周期
  printf(
  ...
  while (_running) 
  {
//1）更新仿真动力学周期、低级控制周期、高级控制周期、期望的仿真速度
    double dt = _simParams.dynamics_dt;                     //计算仿真动力学周期
    double dtLowLevelControl = _simParams.low_level_dt;     //计算低级控制周期
    double dtHighLevelControl = _simParams.high_level_dt;   //计算高级控制周期
    _desiredSimSpeed = (_window && _window->wantTurbo()) ? 100.f : _simParams.simulation_speed;//计算期望的仿真速度
...
//3）正常的运行仿真的步骤迭代
    ...
    {
      _simParams.lockMutex();                         //（1）设置仿真参数的线程上锁
      step(dt, dtLowLevelControl, dtHighLevelControl);//（2）进行步骤迭代【这里非常重要!!!!!!!!!!!!!!!!!！】
      _simParams.unlockMutex();                       //（3）设置仿真参数的线程解锁
      steps++;
    }
//4）非正常的运行仿真的步骤迭代，进入休眠
    else 
    {
...
//5）计算并显示仿真运行时，真实时间、仿真时间，比率，更新三位视图
...
void Simulation::loadTerrainFile(const std::string& terrainFileName,
  ...
  for (auto& key : keys) {
    ...
    if (typeName == "infinite-plane") {
      ...
      R_box.transposeInPlace();  //collisionBox使用“旋转”矩阵“转型”
...
void Simulation::updateGraphics() {
  ...
  _robotDataSimulator->forwardKinematics();  // 计算所有身体位置
