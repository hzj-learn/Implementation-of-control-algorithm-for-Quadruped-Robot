...
void BoundingJumpCtrl<T>::_optimize_jump(const DVec<T> & X0, T xdot_target, T zdot_target )
{
  int N = 31;                  //控制间隔数 
  int N_fc = 15;
  auto opti = casadi::Opti();  // 优化问题

  Slice all;
  // ---- 决策变量 ---------
  auto X = opti.variable(6, N+1);      // 状态轨迹
  auto P = opti.variable(4);           // 着陆位置
  auto F = opti.variable(2, N);        // 反作用力（15：前，1：气，15：后）
  ...
  Dict solver_opts;
  opti.solver("ipopt", plugin_opts, solver_opts);     //设置数字后端 
  auto sol = opti.solve();                            // 实际求解
...
{ 
  ...
  if( i<15 )
  {
    return vertcat (vertcat(x(3), x(4), x(5)), 
        vertcat(u(0)/M, u(1)/M -g), ((P(1) - x(1))*u(0) - (P(0) - x(0)) * u(1))/I );  // 速度
  }
  else
  {
    return vertcat (vertcat(x(3), x(4), x(5)), 
        vertcat(u(0)/M, u(1)/M -g), ((P(3) - x(1))*u(0) - (P(2) - x(0)) * u(1))/I );  //速度
  }
}
