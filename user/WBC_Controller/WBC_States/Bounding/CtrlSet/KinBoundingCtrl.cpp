...
void KinBoundingCtrl<T>::_StatusCheck() 
{
  ...
  if (fabs(_bounding_test->_body_vel[0]) * _nominal_stance_time >_step_length_lim)      //高速边界站姿时间缩放
  {
  ...
  T scale(1.0);
  if ((!_b_front_swing) && (_front_time > (_front_current_stance))) //如果站姿时间结束，切换到摆动
  {
    _b_front_swing = true;
  ...
  if ((!_b_hind_swing) && (_hind_time > (_hind_current_stance))) //如果站姿时间结束，切换到摆动
  {
    _b_hind_swing = true;
