...
/**
 * 功能：初始化MPC提示，成功与否调试，打印出来
 */
void initialize_mpc()
{
  //printf("Initializing MPC!\n");
  if(pthread_mutex_init(&problem_cfg_mt,NULL)!=0)//线程，configuration mutex配置错误
  {
     printf("[MPC ERROR] Failed to initialize problem configuration mutex.\n");
  }   
   
  if(pthread_mutex_init(&update_mt,NULL)!=0)//线程，data mutex.更新错误
  {
...
void setup_problem(double dt, int horizon, double mu, double f_max)
{
  ...
  ////////////////////////////////////（3）重组qp矩阵，把状态方程转换成二次规划QP求解器的表达形式//////////////////////////////////////
...
//{
//  solve_mpc(&update, &problem_configuration);//安全复制问题数据并启动解算器
//}
...
void update_problem_data(double* p, double* v, double* q, double* w, double* r, double yaw, double* weights, double* state_trajectory, double alpha, int* gait)
{
  ...
  //这是安全的，解算器没有运行，并且从同一线程调用update_problem_data和setup_problem
