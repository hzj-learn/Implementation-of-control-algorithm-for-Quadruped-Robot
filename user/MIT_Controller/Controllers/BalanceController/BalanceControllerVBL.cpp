...
{   
   ...
   calc_linear_error();//计算线性化误差

   //计算线性化动力学的矩阵sdot=A*s+B*df
   update_A_LQR();
   update_B_LQR();
   update_P_LQR();//求解连续时间代数Ricatti方程
...
void BalanceControllerVBL::solveQP_nonThreaded(double* xOpt)
{
  ...
  //结合参考控制（f_des）和线性控制（xOpt_eigen）输入
 ...
 * 功能：更新A_LQR
 */
/* NOTE: 我们假设期望角速度为零，如果物体的期望角速度为非零，则需要在_LQR.block公司<3,3>（9,9）也可以解释科里奥利项 */
...
void BalanceControllerVBL::update_B_LQR()
{
  ...
  for(int leg = 0; leg < 4; leg++)
  {
    if(stance_legs == 5 && (leg == 0 || leg==3))  //FR和BL支柱的零B列
    { 
      tempSkewMatrix3.setZero();
    ...
    else if(contact_state(leg) > threshold)     //仅使用接触的支腿计算B_LQR
    { 
      Eigen::VectorXd rd;rd.resize(3,1);
      tempSkewMatrix3.setIdentity();
    ...
    Eigen::VectorXd rd;rd.resize(3,1);  //为QP设置B-触点不变配置
    tempSkewMatrix3.setIdentity();
  ...
  for(int i = 0; i < 3*contact_legs; i++)  //建立权重矩阵R以匹配尺寸
    R_LQR(i,i) = R1_QP(i,i);

  if(stance_legs==5)//增加失去接触支腿的成本
  {
    for(int i = 0; i < 3; i++)
    {
...
void BalanceControllerVBL::update_P_LQR()
{
  ...
  //创建一个包含稳定特征值特征向量的2nxn矩阵U=[U11；U21] 
