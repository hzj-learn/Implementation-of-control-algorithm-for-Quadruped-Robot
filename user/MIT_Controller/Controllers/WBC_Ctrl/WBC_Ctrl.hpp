...
{
  public:
    WBC_Ctrl(FloatingBaseModel<T> model);                     //（1）WBC_Ctrl构造函数
    virtual ~WBC_Ctrl();

    void run(void * input, ControlFSMData<T> & data);         //（2）WBC_Ctrl运行函数
    void setFloatingBaseWeight(const T & weight)              //设置浮计权重函数
    {
      _wbic_data->_W_floating = DVec<T>::Constant(6, weight);
    }

  protected:
    virtual void _ContactTaskUpdate(void * input, ControlFSMData<T> & data) = 0;                    //更新接触任务函数
    virtual void _ContactTaskUpdateTEST(void * input, ControlFSMData<T> & data)
    {
      (void)input;
      (void)data;
    }
    virtual void _LCM_PublishData(){}                                                               //LCM发布数据函数
    void _UpdateModel(const StateEstimate<T> & state_est, const LegControllerData<T> * leg_data);   //更新模型函数
    void _UpdateLegCMD(ControlFSMData<T> & data);                                                   //更新腿部命令函数
    void _ComputeWBC();                                                                             //计算WBC函数
