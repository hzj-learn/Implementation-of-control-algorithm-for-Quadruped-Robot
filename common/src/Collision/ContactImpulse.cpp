...
{
  CC::_nContact = CC::_CheckContact();    //检查有没有接触
  if (CC::_nContact > 0) 
  {
    DVec<T> qdot(_nDof);
    ...
    {
      qdot[i] = state.bodyVelocity[i];    //从状态估计器获取躯干角度
    }
    for (size_t i(0); i < _nDof - 6; ++i) 
    {
      qdot[i + 6] = state.qd[i];          //从状态估计器获取躯干角速度
    }

    _UpdateVelocity(qdot);                //更新
    ...
    {
      // 除以dt将脉冲转换为力，类似于力臂的原理
      CC::_cp_force_list[CC::_idx_list[i]] =CC::_cp_frame_list[i] * CC::_cp_local_force_list[i] / _dt;
    }
  }
}
