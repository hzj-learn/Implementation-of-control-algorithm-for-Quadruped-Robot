/*! @file Cheetah3.h
 *  @brief 构建猎豹3四足动物对象的实用函数
 *此文件基于cheetha3fullrotormodel_mex.m（最初由帕特）并建立了猎豹3号机器人的模型。惯性参数
 *支腿和转子是通过描述的实验程序确定的物理一致惯性参数的线性矩阵不等式
 *认同：一个关于质量分布的统计视角
 */
...
Quadruped<T> buildCheetah3() {
  ...
  //注意，在实际的模拟器中匹配一个小错误是错误的，只是为了检验我得到了正确的答案
