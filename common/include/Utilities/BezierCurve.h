...
class BezierCurve 
{
 public:
  ...
  bool SetParam(T** ctrl_pt, T fin_time)    //（1）设置参数
  {
    _end_time = fin_time;
  ...
  bool getCurvePoint(T u, T* ret)           //（3）获取当前的点
  {
    if (u > _end_time) 
    {
      for (int i(0); i < DIM; ++i) 
  ...
  bool getCurveVelocity(T u, T* ret)        //（4）获取当前的速度
  {
    if (u > _end_time) 
    {
      for (int i(0); i < DIM; ++i) 
