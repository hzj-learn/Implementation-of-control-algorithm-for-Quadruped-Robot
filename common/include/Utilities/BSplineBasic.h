...
class BS_Basic 
{
 public:
   ...
   * ini[3 ~ 5]: vel <br/>
   * ini[6 ~ 8]: acc <br/>
   * @param init : 初始点信息（矢量） <br/>
   * @param fin : 终点信息（矢量） <br/>
   * @param middle_pt   : 中间点信息（矩阵） <br/>
   * @param fin_time : 样条曲线的持续时间 <br/>
   * @return boolean : success <br/>
   */
  bool SetParam(T* init, T* fin, T** middle_pt, T fin_time) 
  {
    _CalcKnot(fin_time);                            //计算
  ...
  /*!
   * 获取给定时间的样条线位置 <br/>
   * 如果输入时间在0之前，则返回初始值 <br/>
   * 如果输入时间在最后一次之后，则返回最后一次 <br/>
   * @param u : time <br/>
